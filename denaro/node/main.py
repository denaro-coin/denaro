...
async def get_address_info(address: str, transactions_count_limit: int = 5, show_pending: bool = False, verify: bool = False):
    ...
    return {'ok': True, 'result': {
        ...
        'transactions': [await transaction_to_json(tx, verify, address) for tx in await db.get_address_transactions(address, limit=transactions_count_limit, check_signatures=verify)] if transactions_count_limit > 0 else [],
